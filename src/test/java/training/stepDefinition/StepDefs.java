...
public class StepDefs {
  ...
  /* @Given("^a user launches \"([^\"]*)\" and navigates to the Adactin web page \"([^\"]*)\"$")
    public void a_user_launches_and_navigates_to_the_Adactin_web_page(String arg1,String arg2) throws InterruptedException {*/
        ...
        web.navigate("https://adactinhotelapp.com/index.php");
    }
    ...
    public void a_user_enters_the_username_and_password_and_clicks_login() throws Exception {
       // adactin.login_Hotel(webUtilities.getWebDriver());

        try{
            set = connect.ConnectAndQuerySQL("jdbc:mysql://localhost:3306/adactin","root","root","Select * from credentials");
            int iRow = connect.rowCount(set);
            for (int i=1;i<=iRow;i++) {
                if (set.next()) {
                    adactin.login_Hotel(webUtilities.getWebDriver(),set);
                }
            }
            set.close();
        }catch (Exception e){
