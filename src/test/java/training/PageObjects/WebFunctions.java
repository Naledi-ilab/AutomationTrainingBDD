...
public class WebFunctions extends WebActions {
    ...
    public void login_Hotel(WebDriver driver,ResultSet rs) {
        hotel_Login log = new hotel_Login(driver);

        try {

            passData(log.txtUsernme, driver,rs.getString("entle123"));
            passData(log.txtPassword, driver,rs.getString("Naledi146"));
 ...
 * Class: WebFunctions
 * Methods: login_Hotel()
 * Passes the credentials from the database to login
 *
 *         search_Hotel()
 *
 *         select_Hotel()
